...
import gxj.study.demo.datastruct.hash.my.BloomFilterStrategies.LockFreeBitArray;
 ...
 * has not actually been put in the {@code BloomFilter32}.
 * <p>
 ...
 *
 * @param <T> the type of instances that the {@code BloomFilter32} accepts
 * @author Dimitris Andreou
 * @author Kevin Bourrillion
 * @since 11.0 (thread-safe since 23.0)
 */
@Beta
public final class BloomFilter32<T> implements Predicate<T>, Serializable {
    /**
    ...
    interface Strategy {
        ...
        /**
         * Identifier used to encode this strategy, when marshalled as part of a BloomFilter32. Only
    ...
    /**
     * The bit set of the BloomFilter32 (not necessarily power of 2!)
     */
    ...
    /**
     * Creates a BloomFilter32.
     */
    private BloomFilter32(
    ...
    /**
     * Creates a new {@code BloomFilter32} that's a copy of this instance. The new instance is equal to
     * this instance but shares no mutable state.
     *
     * @since 12.0
     */
    public BloomFilter32<T> copy() {
        return new BloomFilter32<T>(bits.copy(), numHashFunctions, funnel, strategy);
    }
    ...
    /**
     * Puts an element into this {@code BloomFilter32}. Ensures that subsequent invocations of {@link
     ...
     * true} for an object that has not actually been put in the {@code BloomFilter32}.
     * <p>
     ...
     * case that too many elements (more than expected) have been put in the {@code BloomFilter32},
     * degenerating it.
     *
     ...
     * @since 15.0
     */
    public boolean isCompatible(BloomFilter32<T> that) {
        checkNotNull(that);
        return this != that
     ...
     * @since 15.0
     */
    public void putAll(BloomFilter32<T> that) {
        checkNotNull(that);
        checkArgument(this != that, "Cannot combine a BloomFilter32 with itself.");
        checkArgument(
                this.numHashFunctions == that.numHashFunctions,
    ...
    public boolean equals(@Nullable Object object) {
        ...
        if (object instanceof BloomFilter32) {
            BloomFilter32<?> that = (BloomFilter32<?>) object;
     ...
     * BloomFilter32} with false positive probability 3%.
     * <p>
     * <p>Note that if the {@code Collector} receives significantly more elements than specified, the
     * resulting {@code BloomFilter32} will suffer a sharp deterioration of its false positive
     * probability.
     * <p>
     * <p>The constructed {@code BloomFilter32} will be serializable if the provided {@code Funnel<T>}
     * is.
     * <p>
     ...
     *
     * @param funnel             the funnel of T's that the constructed {@code BloomFilter32} will use
     * @param expectedInsertions the number of expected insertions to the constructed {@code
     *                           BloomFilter32}; must be positive
     * @return a {@code Collector} generating a {@code BloomFilter32} of the received elements
     * @since 23.0
     */
    public static <T> Collector<T, ?, BloomFilter32<T>> toBloomFilter(
    ...
    /**
     * Returns a {@code Collector} expecting the specified number of insertions, and yielding a {@link
     * BloomFilter32} with the specified expected false positive probability.
     * <p>
     * <p>Note that if the {@code Collector} receives significantly more elements than specified, the
     * resulting {@code BloomFilter32} will suffer a sharp deterioration of its false positive
     * probability.
    ...
    public static <T> Collector<T, ?, BloomFilter32<T>> toBloomFilter(
        ...
        return Collector.of(
                () -> BloomFilter32.create(funnel, expectedInsertions, fpp),
                BloomFilter32::put,
                (bf1, bf2) -> {
                    bf1.putAll(bf2);
                    return bf1;
    ...
    /**
     * Creates a {@link BloomFilter32} with the expected number of insertions and expected false
     * positive probability.
     * <p>
     * <p>Note that overflowing a {@code BloomFilter32} with significantly more elements than specified,
     ...
     *                           BloomFilter32}; must be positive
     * @param fpp                the desired false positive probability (must be positive and less than 1.0)
     * @return a {@code BloomFilter32}
     */
    public static <T> BloomFilter32<T> create(
    ...
    @VisibleForTesting
    static <T> BloomFilter32<T> create(
     ...
     */
        ...
        try {
            return new BloomFilter32<T>(new LockFreeBitArray(numBits), numHashFunctions, funnel, strategy);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("Could not create BloomFilter32 of " + numBits + " bits", e);
        }
    }

    /**
     * Creates a {@link BloomFilter32} with the expected number of insertions and a default expected
     ...
     * @return a {@code BloomFilter32}
     */
    public static <T> BloomFilter32<T> create(Funnel<? super T> funnel, int expectedInsertions) {
     ...
     * @since 19.0
     */
    public static <T> BloomFilter32<T> create(Funnel<? super T> funnel, long expectedInsertions) {
    ...
    private static class SerialForm<T> implements Serializable {
        ...
        Object readResolve() {
            return new BloomFilter32<T>(new LockFreeBitArray(data), numHashFunctions, funnel, strategy);
        }
    ...
    /**
     * Writes this {@code BloomFilter32} to an output stream, with a custom format (not Java
     * serialization). This has been measured to save at least 400 bytes compared to regular
     * serialization.
     * <p>
     * <p>Use {@linkplain #readFrom(InputStream, Funnel)} to reconstruct the written BloomFilter32.
     */
     ...
     *                     appear to be a BloomFilter32 serialized using the {@linkplain #writeTo(OutputStream)} method.
     */
    public static <T> BloomFilter32<T> readFrom(InputStream in, Funnel<? super T> funnel)
            throws IOException {
        ...
        try {
            ...
            String message =
                    "Unable to deserialize BloomFilter32 from InputStream."
                            + " strategyOrdinal: "
                            + strategyOrdinal
                            + " numHashFunctions: "
                            + numHashFunctions
